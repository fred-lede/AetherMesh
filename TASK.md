...
## Dashboard — Model Manager Tab ✅ (2026-09-23)
- [x] 需求：Dashboard 新增 Tab，以 UI 管理 models.yaml（新增/編輯/刪除），取代遠端手改檔案
- [x] 規劃：區分本地模型 / 雲端模型；能力多選（Chat/Vision/Image/Reasoning/Tool/Audio/Video/Embedding/Reranker…）；可設定 context window
...
- [x] 設計 spec 已核准並 commit（`docs/superpowers/specs/2026-09-23-model-manager-tab-design.md`）
...
## Phase 37 �X 8002 Anthropic Provider ���d�ˬd�]2026-08-11�^??
- [x] /health 200�]8002 �s���^�F/v1/gpu/status 200�GRTX 5090 32GB + RTX 4070 Ti 16GB healthy�A�� 0 models loaded�B0 workers registered
- [x] POST /v1/messages�]�L tools�^�� **503 overloaded_error�uNo worker was assigned for Ollama provider�v**�]Python probe�FPowerShell �s�u�Q reset�^
- [x] Ollama 122.116.209.1:11434/api/tags �� WinError 10053 �s�u�Q����F/api/chat �� timeout �� **Ollama �D�{���w��/�L�^��**
- [x] 8001�]openai_router�^/health �� timeout �� �æ��P�]����
- [x] agnes custom provider�G�i�F�A�� last request 400 	ool_choice: data did not match any variant of untagged enum ToolChoice�]agnes �Y�� Rust serde json_parse_error�^�X Codex �e�X�� tool_choice value �榡����
- [ ] �ݳB�z�G���� Ollama �� ���� 8001/8002 �� workers ���s���U�Fagnes tool_choice 400 �ݦb��o�e���W�� tool_choice�]�{�ȵL tools �ɭ����^

## Phase 38 — AIIH/ 前綴模型路由修正（muse-glimmer:30b 無法使用）✅ (2026-08-12)
- [x] 現象：`AIIH/muse-glimmer:30b` 請求回退到 `gemma4:31b-it-qat`/`gemma4:e2b`，無法真正使用 muse-glimmer
- [x] 根因：`settings.strip_model_route_prefix` 只剝 `anthropic/` `nvidia_nim/` `ollama_cloud/` `ollama/` `openai/` `gemini/` `xtts/`，**不剝 `AIIH/`（model_alias_prefix）**。`AIIH/muse-glimmer:30b` → registry 比對失敗 → capability routing → `local_model_fallback`（8002 路徑回 gemma4:e2b）；`messages_adapter:162` 再把 `routing_decision.model` 覆寫進 payload → Ollama 實際跑 fallback 模型（`/api/ps` 佐證 192.168.1.200 載入 gemma4:e2b），響應只假性 echo 請求名稱
- [x] 影響範圍：所有 `AIIH/<model>`（無 alias entry）請求都靜默跑 fallback 模型，連 `AIIH/gemma4:26b` 也是
- [x] 修正（`config/settings.py:strip_model_route_prefix`）：prefix 清單加入 `model_alias_prefix()`，改為迭代剝除（`anthropic/AIIH/...` 多層也可全剝）；`resolve_model_alias` 自動繼承（routing_engine.route:344 也用同一函式）
- [x] 修後驗證：`AIIH/muse-glimmer:30b` → `ollama:muse-glimmer:30b` + worker 192.168.1.200（不再 fallback）；`anthropic/AIIH/muse-glimmer:30b` 也正常；`AIIH/gemma4:26b` → `gemma4:26b`（原本也靜默跑 e2b，一併修正）；alias 仍正常（`AIIH/claude-3-5-haiku-agnes-2.0-flash` → agnes）
- [x] `tests/test_routing_engine.py` — 新增 2 tests（strip 剝 alias prefix 多層、alias 前綴模型路由到 registry 而非 fallback）；34 passed；全量 709 passed（22 個既有環境失敗：dashboard_auth / test_security AIIH_API_KEY / test_file_parser 缺 pypdf 等，與本次無關）
- [x] ⚠️ 需重啟 8001/8002 載入修正；已確認 8001 有 /v1/traces、8002 無（anthropic app 未 include）

## Phase 39 — Dashboard OpenCode card Probe 沒反應 ✅ (2026-08-12)
- [x] 現象：Dashboard Providers「PROVIDERS & ROUTING」區 OpenCode 卡片按 Probe 沒反應（agnes/其他卡片正常）
- [x] 根因：`_probe_provider()` 先把名稱 `.lower()`（"OpenCode" → "opencode"）再呼叫 `_probe_custom_provider("opencode")`，但後者對 `settings.load_custom_providers()` 的 dict 做**區分大小寫**的 `data.get("opencode")` → None → 回 `{ok: False, status: "not_found", message: "Provider 'opencode' not found"}`（HTTP 200，JS 顯示 probe failed / 看似沒反應）
- [x] `is_custom_provider()` 本身不區分大小寫，所以先走到 custom probe 才炸
- [x] 修正（`dashboard/dashboard_server.py:_probe_custom_provider`）：dict 未命中時改為大小寫不敏感的 key 掃描，並回傳**正規化 key**（"OpenCode"）
- [x] 驗證：`_probe_provider('OpenCode')` 實測 → `{ok: True, healthy, model_count: 63, latency_ms: 662}`（連 https://opencode.ai/zen/v1）
- [x] `tests/test_custom_providers.py` — +1 test（大小寫不敏感 probe）；32 passed
- [x] ⚠️ 需重啟 dashboard（9001）載入修正

## Phase 40 -??Token ?????�k�s�ƥ�լd + ���� DB ???�����@ (2026-08-12)
- [x] �լd�Gtoken_usage ??38 ??�k�s����]max id 38 -> 1�^�A??�� 29 ?????�����ո�� + �u��y�q
- [x] ??�]�T�{�G	ests/test_usage_export.py:21 ??db.query(TokenUsage).delete() ???�J���� rows�]100/50, 200/60, 300/70, user_id=1, api_key_id=2�^�A��????? 09:11 �����??? import ??�����j��???�u??? config/aiih.db�]���ɥ��] AIIH_DB_PATH�^�� ??�M???�u??? 38 ??����
- [x] ���n�ìV�G	ests/test_dashboard_auth.py ??13:18 ???�u??? DB ???�J���� user me/lex ??api_keys 3/7�]import ??�� bug�Gengine ?? module ?? import ??�j�w�^
- [x] �ץ� 1�G	ests/conftest.py ????? conftest ?? import ??�j�� AIIH_DB_PATH ���V tempfile �� ???����� process �ä��I???�u??? DB
- [x] �ץ� 2�G	ests/test_usage_export.py ??���]�] temp DB path�]defense in depth�A��W�]�]�w���^
- [x] ??�b???�u??? DB�G�R���� users me/alex�Bkeys 3/7�Btoken_usage ids 1-3 �� ��_ users=1, api_keys=5, token_usage=26 ??�u???����
- [x] ���ҡGpytest tests/test_usage_export.py ??passed�F���ի�u??? DB mtime ??COUNT ����??��

## Phase 41 -?? MinerU PDF �����X�]�ǳƤ��^ (2026-08-12)
- [x] �����G�A�X����Ω� builtin tool�]lazy import / subprocess�A���i�֤ߨ̿�^
- [x] ??�� .venv312�]Python 3.12.8�^�W�����ҡ]�D .venv �O 3.14�AMinerU �u�䴩 3.10-3.13�^
- [x] �w�� mineru 3.4.4�]�s���� onnxruntime�A??�� torch�^??CLI ���ҡGmineru -p <file> -o <out> -m auto -b pipeline
- [ ] ��������|�q modelscope ??�� layout/OCR ??���]�ݺ����^
- [ ] ��@ untime/tools/builtin/document.py�]document_to_markdown tool�^+ Settings ��� + ����

## Phase 41 -?? MinerU PDF �����X (2026-08-12)
- [x] ??�� .venv312�]Python 3.12.8�^�W�����ҡF�D .venv 3.14 ????MinerU �u�䴩 3.10-3.13
- [x] �w�� mineru 3.4.4 + mineru[pipeline] extras + torch 2.11.0+cu128�]RTX 5090 CUDA �i�Ρ^
- [x] ??�]�ƹp�G(1) mineru.exe launcher shebang ???�~���V uv 3.14 �� ??�� .venv312\Scripts\python.exe -m mineru.cli.client�F(2) ???venv ???���i�{�~�� PYTHONHOME/UV_INTERNAL__PYTHONHOME�]uv 3.14�^�� 3.12 �l�i�{ stdlib ???3.14 ???�a �� converter ?? subprocess env ???��???��???�ܼ�
- [x] ??????untime/documents/mineru_converter.py�]subprocess CLI wrapper�A��X markdown + meta�^
- [x] ??????untime/tools/builtin/document.py�Gdocument_to_markdown tool�]lazy import�A??�]�T�ΡAAIIH_MINERU_ENABLED=true ???�}�^
- [x] ??????document_server/document_server.py�GREST API�]port 9500�AAIIH_DOCUMENT_PORT�^
- [x] Settings ???��Gmineru_enabled/python/backend/method/timeout + document_port/api_key�F.env.example ???�s
- [x] Launcher ???�J document_server service
- [x] ���աGtests/test_document_tool.py ??9 passed�]mock�A�s�~���̿�^
- [x] E2E�G???�u??? PDF ??pipeline backend �� markdown ���\�]�t�����ҫ��U���A~65s�^�FAPI TestClient �W�ǩ�����\
- [x] document server ??�b 9500�]logs/document_server.log�^
- [ ] ��L agent ??�J�GPOST /v1/documents/extract�]multipart file�^�F/health ???�ˬd�F�i?? AIIH_DOCUMENT_API_KEY ???�v
- [ ] ��ĳ�G??�ݱҥ� agent tool ???�] .env AIIH_MINERU_ENABLED=true ???���� launcher
- [x] �M�w���[�J Dashboard �d���]�����[�^�A�O���{���AAPI /v1/documents/health �w�����ʱ�
- [x] 远端 agent 接入验证：8001 /v1/documents/health 需 AIIH_API_KEY，mineru_available=true，include_images query param 已生效

### ⚠️ 更正（2026-08-13）：document server 9500 從未存在
- TASK.md 先前三行錯誤記錄已更正，實際架構與此不符：
  - ❌ `document_server/document_server.py`（port 9500 / `AIIH_DOCUMENT_PORT`）— **目錄與檔案不存在**
  - ❌ Launcher 加入 document_server service — `runtime/launcher/launcher.py` 的 `SERVICE_DEFS` **無此項**
  - ❌ document server 運行於 9500（logs/document_server.log）— `logs/` 目錄不存在
- ✅ 實際架構：`router/document_router.py` 直接掛載於 8001 `openai_router`（`openai_router.py:119`），`/v1/documents/*` 與 LLM 路由共用同一 process；MinerU 由 `_mineru_command()` 以 `settings.mineru_python`（.venv312 / 3.12）spawn subprocess CLI 執行，3.12 **不監聽任何 port**

## Phase 42 — Document API out_dir 累積 bug 修復 ✅ (2026-08-13)
- [x] 現象：`image_count` 一路漲（7305 → 7914），因為 `/v1/documents/extract` 未指定 out_dir，所有請求共用 `Temp/mineru_out/`，`_discover_images()` 遞迴掃到舊轉換的圖片
- [x] 修正（`router/document_router.py`）：
  - `/extract`（upload）：改用 `tempfile.mkdtemp(prefix="aethermesh_mineru_")` 獨立 out_dir，finally 內 `shutil.rmtree` 清理（含 tmp 檔）
  - `/extract/json`：未給 `output_dir` 時同樣用 mkdtemp 獨立目錄；有給則保留使用者目錄不清理；finally 不再用 return（避免吞例外）
- [x] 工具路徑（`runtime/tools/builtin/document.py`）保留原意：輸出留在原始檔旁
- [x] 驗證：tests/test_document_router.py + tests/test_document_tool.py 15 passed
- [x] E2E：重啟 8001 後 `include_images=true` → image_count=1（正確）、output_path 指向獨立 aethermesh_mineru_* 目錄、temp 無殘留
- [x] ⚠️ 發現 8001 有重複 uvicorn process（舊 .venv + 新 uv 3.14），已停掉舊 PID 19532 重啟新版（現 PID 24160）

## Phase 42b — include_images 實際未嵌入 base64 bug 修復 ✅ (2026-08-13)
- [x] 現象：`include_images=true` 回傳的 markdown 仍是 `![](images/xxx.jpg)` 相對路徑，**沒有** `data:image/` base64 — 遠端用戶看不到圖
- [x] 根因（`runtime/documents/mineru_converter.py:_embed_images_in_markdown`）：用 `out_dir / rel` 解析圖片，但 MinerU 實際輸出在 `out_dir/<檔名>/auto/images/*.jpg`（與 markdown 同層）→ `img_path.exists()` 永遠 False → 原樣回傳
- [x] 修正：函式改收 `md_path`，以 `md_path.parent` 為基準解析 `images/xxx.jpg`
- [x] `tests/test_document_tool.py` — +2 tests（真實轉換嵌入 base64、缺圖保留原連結）
- [x] 驗證：test_document_tool 11 passed + test_document_router 6 passed
- [x] E2E：重啟 8001（PID 16272）→ back.pdf `image_count=1`、markdown 含 `data:image/jpeg;base64,...`（4292 chars），`sample_output.md` 可直接 VS Code 預覽圖片

## Phase 42c — Document API 異步 job 模式 ✅ (2026-08-13)
- [x] 問題：大檔（4MB / 600頁 / 50MB）同步轉換需 10-30 分鐘，遠端 agent 的 HTTP client timeout（60-120s）中途放棄 → 「連線失敗」
- [x] 調查結論：timeout 已是 900s（不需調）、GPU 充足（5090 16G/32G）、每次轉換固定 ~34s 冷啟動（subprocess 載入模型）→ 根因是同步長請求 + client timeout
- [x] `runtime/documents/job_manager.py` — `DocumentJobManager`：串行 queue（單 worker，GPU 不並發爆炸）+ 背景 worker thread + sweeper TTL 清理（3600s）+ 狀態機（queued/processing/completed/failed）+ ISO timestamp + 內部欄位隔離
- [x] `router/document_router.py` — 新端點：
  - `POST /v1/documents/extract/async`（multipart upload）→ 立即回 `{job_id, status: queued}`
  - `POST /v1/documents/extract/json/async`（path）→ 同上，支援 include_images/backend/method/timeout_s
  - `GET /v1/documents/jobs/{job_id}` → 輪詢狀態（completed 含 result.markdown / failed 含 error）
  - `GET /v1/documents/jobs` → 最近 job 列表
- [x] `tests/test_document_jobs.py` — 11 tests（submit/get/list/執行/失敗/清理/TTL/ISO 時間/背景 worker/串行化）
...
- [x] Watchdog 誤判修復（2026-08-24）：openai_router 08:52 重啟後啟動期（Tavily/Serper 初始化重試）被誤判為無回應連發告警。新增 (1) startup_grace_s（預設 180s，process 啟齡內不報 dead/hang 告警，psutil create_time 判斷）；(2) exclude_services 清單（完全跳過監控+告警+重啟）。Dashboard 面板新增「Monitored services」勾選區與 Startup grace 欄位；example 更新；+4 tests（19 watchdog 全過）
- [x] 服務開關與通知語意修正（2026-08-24）：新增 config/services.json 期望狀態檔——Dashboard「Service Control」卡片（GET/PUT /api/services，admin）切換服務開關；launcher 每 1s reconcile（停用→stop，啟用→start）；被主動停止的 ServiceProcess 標記 intentionally_stopped，watchdog 完全跳過（不監控不告警不重啟）。釐清：openai_router=8001 是 OpenAI 相容總入口（本地模型也走），缺 OPENAI_API_KEY 不影響啟動。+16 tests（launcher 10、services API 6、watchdog +2 intentional）＝全量 797 passed / 20 failed 與基準一致
- [x] Service Control 已提交推送（2026-08-24）：309cf6c（models.yaml）+ 33f204a（Service Control + startup grace，15 files +527）。config/services.json 已 gitignore（範本 services.json.example 入庫）
- [x] Ollama 深度健康檢查（2026-08-24）：新增 runtime/health/ollama_probe.py——1-token 推論探測（淺層 /api/tags 測不出 runner 死亡）；watchdog 整合 ollama_deep_check 設定（enabled/base_url/interval_s=120/timeout_s=30/model/failures_to_alert=2，mtime 熱重載），連續失敗告警+恢復 INFO，閒置（無載入模型）視為健康；summary 增加ollama 節點。+11 tests（probe 7 + watchdog 整合 4）。待 commit
- [x] 整棧自動復活 & 開機啟動（2026-08-28）：診斷發現系統停擺根因=AetherMesh 服務棧整組沒跑（Ollama 反而正常），launcher 前景執行+無執行時限排程→整組掛掉就靜默等死。新增：(1) 獨立 supervisor（runtime/launcher/supervisor.py）監控 launcher pid+sentry ports，死掉自動重拉 start_all.bat，60s anti-crash-loop cooldown，log 到 logs/launcher_supervisor.log；(2) runtime/supervisor_util.py probe_port/probe_http；(3) launcher 寫 launcher.pid + launcher_sentry.json（活躍服務 port）供 supervisor 判活；(4) python -m runtime.launcher supervise entry point；(5) scripts/start_supervisor.bat；(6) 重設 AIIH-Platform 排程任務→執行 start_supervisor.bat、ExecutionTimeLimit=PT0S（原 PT72H 一天後就會被砍）、S4U logon、restart-on-failure。+11 supervisor tests；全量 819 passed/20 failed 與基準一致零回歸
- [x] 已提交推送 59394e8（2026-08-28）：standalone supervisor + 開機自癒。系統已透過 supervisor 自動復活（所有 7 服務聆聽中）
