...
## 2026-09-22 — Context-length completion + opencode/Telegram integration
...
- `runtime/orchestration/openai_handler.py` + `anthropic_converter.py`: `list_models` reads `config/models.yaml` fresh on every request (was a startup snapshot) and exposes `context_length`/`context_window` in each model's metadata — any OpenAI-compatible client can now read the true window instead of guessing.
...
- User reported Telegram still showing 200k for `z-ai/glm-5.3`. Root cause traced through the bot's `@grinev/opencode-telegram-bot` source:
  ...
  - The bot never reads AetherMesh's `/v1/models` — it only talks to the opencode server, whose model specs come from opencode.jsonc / models.dev.
- Fix was client-side config, not AetherMesh code: added `"limit": {"context": 1047576, "output": 131072}` (+ `reasoning`/`tool_call`) to `z-ai/glm-5.3`, `z-ai/glm-5.3-flash` under the `aiih` and `aiih2` providers in `~/.config/opencode/opencode.jsonc`. Verified working — Telegram now shows 1M context.
...
## 2026-09-23 — Dashboard Model Manager tab (CRUD for models.yaml)
- New `runtime/orchestration/model_registry_store.py`: loads `config/models.yaml`, mtime-cached `get_models()`, atomic `save_models()` (`<file>.tmp` → `os.replace`, `PermissionError` retry ×5 with backoff, `.bak` backup, other top-level keys preserved), plus `normalize_model()` / `validate_model()` (name regex `^[A-Za-z0-9._:/+-]+$`, provider whitelist, local requires ≥1 valid `worker_bindings` port, positive `context_length`, capability whitelist) and `is_local_model()`.
- New `runtime/orchestration/model_references.py`: `scan_model_references()` / `update_model_references()` over `routing_rules.yaml` (aliases, fallbacks, rules) for safe model renames.
- Hot reload: `RouterService._ensure_registry()` and `AnthropicRouter._ensure_registry()` refresh their startup snapshots from `settings.model_registry()` when the models.yaml mtime changes (called from the chat/responses/list entry points and `_to_openai_payload`). `routing_engine.route()` already reads models fresh per request, so it needed no change (a planned mtime hook was reverted — it clobbered `_routing_rules`).
- Capabilities: `providers/registry.py` adds `image_gen` and `video`; the `image` alias now maps to `image_gen` (was `vision`). Annotation/UI only this phase — no routing scores.
- Dashboard: new admin **Models** tab (list with local/cloud + provider + category filters, capability badges, workers, context) and an edit drawer (name, category, provider, worker bindings, capability checkbox grid, context window + auto-fetch, rename flow with the three-way reference choice). Endpoints: `GET/POST /api/models`, `PUT/DELETE /api/models/{name}`, `POST /api/models/{name}/fetch-context`, `POST /api/models/reload`, `GET /api/models/{capabilities,providers,nodes}`; mutations call `_require_admin(request)`.
- Tests: `tests/test_providers_registry.py` 4, `tests/test_model_registry_store.py` 16, `tests/test_model_references.py` 3, `tests/test_model_hot_reload.py` 3, `tests/test_models_api.py` 12, `tests/test_models_ui.py` 2 (structural). No new failures among pre-existing env cases.
- Note: `templates/index.html` pre-compiles at dashboard import, so the dashboard (9001) needs a restart to serve the new tab; JS is disk-served.
