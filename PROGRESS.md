...
ame Äæ¦ì¥¿³W¤Æ¦¨ function object¡C®M¥Î©ó _normalize_payload_for_provider¡]ÂĞ»\ chat/streaming/responses + custom provider¡^»P handle_responses openai passthrough ¤À¤ä
...
- ?? »İ­«±Ò 8002¡]anthropic_router¡^¸ü¤J¡F8001 ¥ç»İ­«±Ò¡]²{¬° timeout¡^¡C¥t Ollama ¥Dµ{¦¡»İ¥ı«ì´_¡A§_«h Ollama ¸ô¥Ñ¤´ 503
...
## 2026-08-23 â€” Phase 43: Watchdog + Telegram/Synology Chat å‘Šè­¦ + è¨˜æ†¶é«”æ²»ç†
- æ–°å¢ runtime/alerting/ï¼ˆNotifier ABCã€TelegramNotifierã€SynologyChatNotifierã€AlertManagerï¼šper-rule cooldown / min_severity / mtime ç†±é‡è¼‰ / æ¸¬è©¦ç™¼é€ï¼‰
- æ–°å¢ runtime/health/watchdog.pyï¼šlauncher å…§èƒŒæ™¯ threadï¼Œper-service process alive + /health hang æ¢æ¸¬ + psutil RSS + ç£ç¢Ÿç©ºé–“ï¼›auto-restart å¯è¨­å®š restart_after_s / cooldown_s / max_per_day / excludeï¼Œé”ä¸Šé™ç™¼ CRITICAL
- Launcher æ•´åˆï¼šrestart_service() å…¬é–‹æ–¹æ³•ï¼›start_all è‡ªå‹•å•Ÿå‹• watchdogã€stop_all åœæ­¢
- Dashboardï¼šSystem tab æ–°å¢ã€ŒNotifications & Watchdogã€admin é¢æ¿ï¼ˆé€šé“è¨­å®š + æ¸¬è©¦æŒ‰éˆ• + watchdog/auto-restart åƒæ•¸ï¼‰ï¼›API GET/PUT /api/notificationsï¼ˆsecrets é®ç½©ï¼‰ã€POST /api/notifications/test/{channel}
- è¨­å®šï¼šconfig/notifications.jsonï¼ˆgitignoredï¼‰+ notifications.json.exampleï¼›dashboard å­˜æª”å…é‡å•Ÿç”Ÿæ•ˆï¼ˆmtime ç†±é‡è¼‰ï¼‰
- Phase 33 ç„¡ç•Œè¨˜æ†¶é«”ä¿®å¾©ï¼šmetrics histogramsã€event durationsï¼ˆdeque maxlen=1000ï¼‰ã€episodic recordsï¼ˆ5000ï¼‰ã€rate_limiter buckets TTL æƒæã€shared_memory broadcast logï¼ˆ1000ï¼‰ã€routing_engine worker health cache é©…é€
- æ¸¬è©¦ï¼štest_alerting.py 17 + test_watchdog.py 12 æ–°å¢ï¼›test_memory.py åŠ  session_store éš”é›¢ fixture ä¿®å¾©è·¨é‹è¡Œæ±¡æŸ“
- å…¨é‡ 776 passed / 20 failedï¼ˆstash å°ç…§è­‰å¯¦å¤±æ•—é›†åˆèˆ‡ä¹¾æ·¨ codebase å®Œå…¨ä¸€è‡´ï¼Œçš†ç‚ºæ—¢æœ‰ç’°å¢ƒ/æ±¡æŸ“å•é¡Œï¼‰
