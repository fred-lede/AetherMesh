...
from router.server_tool_policy import forced_server_tool
...
def run_web_search(query: str, max_results: int, timeout_s: int) -> list[dict[str, str]]:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "AIIH/1.0 (+local web_search server tool)"},
    )
...
def run_web_fetch(url: str, timeout_s: int) -> dict[str, str]:
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "AIIH/1.0 (+local web_fetch server tool)"},
    )
    with urllib.request.urlopen(request, timeout=timeout_s) as response:
        media_type = response.headers.get_content_type() or "text/plain"
