...
from router.capabilities import required_openai_capabilities
from router.content_blocks import content_part_to_text_and_images, normalize_image_ref
...
class RouterService:
    ...
    C_FALLBACK_MAP = {
        # Tier 1: 5090 -> 4070Ti (12B) -> P40 (9B/4B)
    ...
    def _resolve_provider_and_worker(
        ...
        *,
        ...
        # Define the chain of models to try (starting with the requested one)
        ...
        for current_model in try_models:
            provider = (payload.get("provider") or self._provider_for_model(current_model)).lower()
            if provider in ("openai", "gemini", "nvidia_nim", "ollama_cloud"):
                # Cloud providers are handled directly by their adapters
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if dispatch.get("status") == "assigned":
                        # Update payload with the actual model used for telemetry
                        payload["model"] = current_model
                        return provider, dispatch["worker"]
                    else:
                        # If it was queued but we needed sync, treat as a failure for this worker
                        last_error_resp = response
                elif response.status_code in {429, 503}:
                    last_error_resp = response
                else:
                    # Other errors (500, etc.) stop the fallback chain immediately
                    raise self._as_http_error(status_code=502, code="control_plane_error", message=response.text)

            except requests.RequestException as exc:
                last_error_resp = exc
                if current_model == original_model:
                    # If the primary fails with a network error, we still try fallbacks
                    pass 

        # If we reach here, all models in the chain failed or weren't available
    ...
    def _adapter(self, provider: str, worker: dict[str, Any] | None):
        if provider == "ollama":
            if worker is None:
                raise self._as_http_error(status_code=503, code="worker_unavailable", message="No worker was assigned.")
        ...
        raise self._as_http_error(status_code=400, code="unsupported_provider", message=f"Unsupported provider: {provider}")
    ...
    def _apply_generation_defaults(self, payload: dict[str, Any]) -> dict[str, Any]:
        # Relay behavior: do not rewrite generation controls.
        return payload
    ...
    def _enqueue_async_task(self, endpoint: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        provider = (payload.get("provider") or self._provider_for_model(model)).lower()
        ...
        *,
        ...
        if worker is not None:
            try:
                ...
                pass
        try:
            # Note: In a real environment, we would pass these to the control plane,
            # and the control plane would call metrics_store.record_request.
            # Since telemetry is a simple POST to the control plane, we keep it as is,
            # but ensure the control plane's /cluster/telemetry endpoint
            # (which calls metrics_store.record_request) is updated.
            get_session().post(
...
app = FastAPI(title="AI Inference Hub Router", version="4.0.0")
